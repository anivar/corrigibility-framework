...
\node[box, right=of llm] (action) {Proposed Action\\Envelope};
...
% Action Boundary - vertical separator
\draw[dashed, very thick, blue!70] ($(action.east)+(0.45,1.2)$) -- ($(action.east)+(0.45,-2.0)$);
\node[font=\scriptsize\sffamily\bfseries, text=blue!70, rotate=90, anchor=south] at ($(action.east)+(0.7,-0.4)$) {ACTION BOUNDARY};

% GOVERN label
\node[below=0.1cm of validator, font=\tiny\sffamily, text=green!50!black] {(GOVERN)};

\end{tikzpicture}
\caption{\textbf{Action Boundary Protocol.} Probabilistic inference (orange) produces proposed action envelopes. A deterministic validator (green, implementing GOVERN) enforces hard constraints before execution. Invalid actions are rejected, logged, and fed back for learning. The vertical \textbf{Action Boundary} separates the inference domain from the execution domain---the architectural instantiation of corrigibility in agentic systems.}
