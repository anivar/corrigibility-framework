...
\begin{abstract}
While deterministic infrastructure is governed by the Rule of the Code, AI-mediated administration is governed by the \textbf{Rule of the Workflow}. This paper establishes structural requirements for \textbf{Epistemic Public Infrastructure (EPI)}---systems where state decisions emerge from learned parameters rather than explicit logic.

We demonstrate that ``AI Alignment'' (operator control) is insufficient for public legitimacy; what is required is structural \textbf{corrigibility} (affected-subject correction). The companion paper's five tests remain invariant; verification methods adapt.

\textbf{Key concepts}: (1) \textbf{LWD-R}---four-layer transparency extending CODE to training pipelines; (2) \textbf{Variety Drift}---why AI corrigibility is a persistence condition; (3) \textbf{Compute Capture}---when ``open weights'' fails FORK; (4) \textbf{WCC}---quantifying epistemic delegation risk; (5) \textbf{Action Boundary}---deterministic envelope around stochastic inference.

By enforcing the Action Boundary, we ensure that while the ``thought'' remains probabilistic, the ``act'' remains legally corrigible. The framework moves AI governance from ethics to engineering.
\end{abstract}
...
\paragraph{Why Four Layers?} The first three (L, W, D) are familiar from ``open source AI'' discourse. The fourth, \textbf{Representation (R)}, is this framework's contribution. When an AI system classifies citizens as ``eligible'' or ``risky,'' it instantiates categories that become administrative facts. These categories---the model's ontology---are often hidden in the latent space, undocumented and non-contestable. Failure to disclose \textbf{R} means the model's worldview is structurally opaque, even if weights are published.
...
\paragraph{The Problem.} A welfare eligibility model classifies applicants into ``low-risk'' and ``high-risk.'' A fraud detection system labels transactions as ``suspicious'' or ``normal.'' A world model parses geopolitical actors into ``threat'' categories. These classifications are not neutral descriptions; they are \textit{definitional acts} that shape administrative reality. If the categories themselves cannot be challenged---if no one outside the vendor can ask ``why is this person high-risk?'' and receive a contestable answer---then the state has surrendered its interpretive sovereignty.
...
\paragraph{Why This Matters.} Without mechanisms for retraining accessible to the community, variety drift widens indefinitely. A model that passed all five corrigibility tests at deployment may fail them two years later---not because anyone changed anything, but because the world moved and the model didn't. \textbf{AI corrigibility is not a deployment certification; it is a persistence condition.}
...
The engineering instantiation of this protocol is the \textbf{Agentic Skill}.

\begin{definition}[Agentic Skill]
An Agentic Skill is a bounded, machine-readable orchestration protocol that encapsulates stochastic inference within the Action Boundary. It acts as the technical instantiation of the Agentic API Contract.
\end{definition}

The open \texttt{SKILL.md} standard (agentskills.io), adopted by 18+ AI agent platforms including OpenAI Codex, GitHub Copilot, and Cursor, provides the concrete specification. A skill is a directory containing a \texttt{SKILL.md} file with YAML frontmatter and Markdown instructions:
...
\begin{itemize}[noitemsep]
    \item \textbf{Server Edge (Federated Nodes)}: Running infrastructure across multiple independent server nodes (similar to Estonia's X-Road) ensures no single operator maintains execution monopoly. This creates Functional Exit Equivalence (FEE): if one node is captured, system variety is preserved through other federated nodes.
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{For technologists}: Agentic Skills offer a practical architecture for building corrigible AI systems. The \texttt{allowed-tools} constraint and reproducible skill definitions satisfy GOVERN without requiring interpretability of model internals.
...
The EPI extension framework, including JSON schemas for agentic workflow assessment, is available at:

\begin{itemize}
    \item EPI framework documentation: \url{https://anivar.net/corrigibility/epi/}
    \item Infrastructure manifest schema (EPI): \url{https://anivar.net/corrigibility/epi/schema/infrastructure.json}
    \item Corrigibility assessment schema (EPI): \url{https://anivar.net/corrigibility/epi/schema/corrigibility.json}
    \item Companion DPI framework: \url{https://anivar.net/corrigibility/dpi/}
    \item Paper source and repository: \url{https://github.com/anivar/corrigibility-framework}
\end{itemize}
