...
\begin{tikzpicture}[
    node distance=0.8cm,
    box/.style={rectangle, draw, rounded corners, minimum height=1cm, minimum width=2.2cm, font=\small\sffamily, align=center},
...
% Labels
\node[above=0.3cm of llm, font=\tiny\sffamily, text=orange!70!black] {Stochastic};
\node[above=0.3cm of validator, font=\tiny\sffamily, text=green!50!black] {Deterministic};
\node[above=0.3cm of exec, font=\tiny\sffamily, text=green!50!black] {Deterministic};

% Boundary annotation
\draw[dashed, thick, blue!50] ($(action.north west)+(-0.2,0.5)$) -- ($(action.north east)+(0.2,0.5)$);
\node[above=0.6cm of action, font=\tiny\sffamily\bfseries, text=blue!70] {ACTION BOUNDARY};

\end{tikzpicture}
\caption{\textbf{Action Boundary Protocol.} Probabilistic inference (orange) produces proposed actions. A deterministic validator (green) enforces hard constraints before execution. Invalid actions are rejected and logged. The action boundary separates epistemic generation from authorized execution.}
