...
\begin{abstract}
Digital Public Infrastructure (DPI) is frequently described in terms of openness, interoperability, inclusion, or scale. These descriptors, while normatively appealing, do not specify structural conditions under which systemic error can be detected and corrected by affected participants. This paper proposes corrigibility as a necessary structural precondition for legitimate digital public infrastructure.

Corrigibility is defined as the property of a system in which affected participants possess guaranteed mechanisms to detect, contest, and override systemic error. Drawing on control theory, commons governance, and distributed systems design, the paper formalizes five jointly necessary conditions for structural corrigibility: inspectable logic (CODE), independent verifiability (AUDIT), reversible participation (EXIT), binding participatory constraint (GOVERN), and independent reproducibility (FORK). These conditions operate across observability, participation, constraint, and replacement layers. The absence of any single condition converts the infrastructure into an open-loop system in which error accumulation cannot be structurally bounded.

The framework distinguishes structural corrigibility from transparency, accountability, or advisory governance in isolation, and provides a testable basis for evaluating real-world DPI systems. Corrigibility does not guarantee optimal performance or fairness; rather, it establishes the minimal feedback architecture required for sustained systemic correction at population scale.
\end{abstract}

\begin{keywords}
Corrigibility, Digital Public Infrastructure, Cybernetics, Feedback Control, Requisite Variety, Infrastructure Accountability, India Stack
\end{keywords}

\section*{Executive Summary}

Digital Public Infrastructure (DPI) is often evaluated using aspirational criteria such as openness, interoperability, inclusion, or scale. However, these descriptors do not answer a more fundamental question: can the system be corrected when it goes wrong?

This paper argues that the defining feature of legitimate public infrastructure is \textbf{corrigibility}: the structural ability of affected participants to detect, contest, and override systemic error. Without this property, digital infrastructure becomes an open-loop system: errors may be acknowledged, but correction remains discretionary rather than guaranteed.

The paper identifies five structural conditions required for corrigibility:
...
These conditions operate across observability, participation, governance, and replacement layers. The absence of any one condition creates structural asymmetry and permits unbounded error accumulation.

Corrigibility does not guarantee fairness, efficiency, or optimal design. It establishes the minimal feedback architecture required for sustained public control. The framework provides a testable basis for evaluating real-world DPI systems and distinguishes structural accountability from transparency or advisory governance alone.
...
Corrigibility operates across four control layers. Each layer addresses a distinct failure mode. Figure~\ref{fig:control-loop} illustrates this architecture as a standard feedback control system.
...
Together, these conditions form a closed-loop structure. Removal of any one condition reintroduces structural asymmetry and permits unbounded error accumulation.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
    layer/.style={rectangle, draw, rounded corners, minimum width=6.5cm, minimum height=0.9cm, font=\sffamily\small, align=center},
    test/.style={rectangle, rounded corners, draw=#1, fill=#1!20, minimum width=1.2cm, minimum height=0.5cm, font=\tiny\bfseries},
    arrow/.style={-{Stealth[length=2mm]}, thick, gray},
    failure/.style={font=\tiny\sffamily, text=red!70!black, align=left}
]

% Title
\node[font=\sffamily\bfseries, text=gray] at (0, 5) {CLOSED-LOOP CORRIGIBILITY STRUCTURE};

% Layers (bottom to top)
\node[layer, fill=forkcolor!10] (replacement) at (0, 0) {\textbf{Replacement Layer}};
\node[layer, fill=governcolor!10] (constraint) at (0, 1.3) {\textbf{Constraint Layer}};
\node[layer, fill=exitcolor!10] (participation) at (0, 2.6) {\textbf{Participation Layer}};
\node[layer, fill=auditcolor!10] (observability) at (0, 3.9) {\textbf{Observability Layer}};

% Tests
\node[test=forkcolor, right=0.2cm of replacement.east, anchor=west] {FORK};
\node[test=governcolor, right=0.2cm of constraint.east, anchor=west] {GOVERN};
\node[test=exitcolor, right=0.2cm of participation.east, anchor=west] {EXIT};
\node[test=codecolor, right=0.2cm of observability.east, anchor=west] (code) {CODE};
\node[test=auditcolor, right=0.1cm of code.east, anchor=west] {AUDIT};

% Failure consequences (right side)
\node[failure, right=2.8cm of replacement.east, anchor=west] {If removed: \textbf{Permanent capture}\\(no alternative exists)};
\node[failure, right=2.8cm of constraint.east, anchor=west] {If removed: \textbf{Arbitrary power}\\(correction is discretionary)};
\node[failure, right=2.8cm of participation.east, anchor=west] {If removed: \textbf{Coercive participation}\\(no exit signal)};
\node[failure, right=2.8cm of observability.east, anchor=west] {If removed: \textbf{Opaque error accumulation}\\(drift undetectable)};

% Arrows showing dependency (upward flow)
\draw[arrow] (replacement.north) -- (constraint.south);
\draw[arrow] (constraint.north) -- (participation.south);
\draw[arrow] (participation.north) -- (observability.south);

% Closed-loop arrow (feedback from top back to bottom)
\draw[arrow, gray!60, dashed, thick]
    (observability.east) -- ++(0.8,0)
    arc[start angle=90, end angle=-90, radius=2.6cm]
    -- (replacement.east);
\node[font=\tiny\sffamily, text=gray!70, rotate=-90, anchor=south] at (4.3, 2.0) {Feedback loop};

% Open-loop warning
\node[font=\scriptsize\sffamily, text=red!60!black, align=center] at (0, -0.8) {Removal of any layer $\rightarrow$ \textbf{Open-loop system}};

\end{tikzpicture}
\caption{\textbf{Closed-Loop Corrigibility Structure.} Each layer addresses a distinct failure mode, with specific power asymmetries that emerge when removed. Observability enables error detection; Participation enables error signaling; Constraint enables correction; Replacement ensures corrective capacity cannot be permanently blocked. \textbf{Removal of any layer converts the system to open-loop}, where error accumulation is unbounded.}
\label{fig:four-layers}
\end{figure}

Figure~\ref{fig:control-loop} illustrates the complete feedback architecture.
...
This paper argues that corrigibility is a necessary structural condition for public legitimacy in digital infrastructure. It does not claim that corrigibility guarantees optimal performance, fairness, or efficiency. Corrigibility bounds systemic error; it does not eliminate epistemic or operational imperfection. The absence of corrigibility, however, renders sustained correction structurally impossible.
...
\begin{itemize}
    \item \textbf{Repository}: \url{https://github.com/anivar/corrigibility-framework}
    \item \textbf{Agentic Skills} (following the Agentic Skills specification \citep{agentskills2025}):
    \begin{itemize}
        \item \texttt{skills/schema-creator/}: For operators (infrastructure.json manifests) and auditors (corrigibility.json assessments). Supports both DPI and EPI with role-specific templates.
        \item \texttt{skills/schema-evaluator/}: Validates published schemas, verifies score calculations, and detects contradictions between operator claims and auditor findings.
        \item \texttt{skills/corrigibility-assessment/}: Rules and test definitions. Decomposes systems into architectural layers (identity, credential, payment, data\_exchange, registry) and applies the five tests.
    \end{itemize}
    \item \textbf{DPI Schemas}: \texttt{schema/dpi/infrastructure.json}, \texttt{schema/dpi/corrigibility.json}
    \item \textbf{EPI Schemas}: \texttt{schema/epi/infrastructure.json}, \texttt{schema/epi/corrigibility.json} (see companion paper)
\end{itemize}
...
Researchers and auditors may access, validate, and contribute to these schemas via the permanent repository: \url{https://anivar.net/corrigibility/dpi/schema/}. The framework source code, including validation scripts suitable for CI/CD integration, is available at the GitHub repository listed in the Data and Code Availability section.
