...
\centering
\begin{tikzpicture}[
    layer/.style={rectangle, draw, rounded corners, minimum width=8cm, minimum height=0.9cm, font=\sffamily\small, align=center},
...
\node[layer, fill=governcolor!10] (constraint) at (0, 1.2) {\textbf{Constraint Layer}};
\node[layer, fill=exitcolor!10] (participation) at (0, 2.4) {\textbf{Participation Layer}};
\node[layer, fill=auditcolor!10] (observability) at (0, 3.6) {\textbf{Observability Layer}};

% Tests
\node[test=forkcolor, right=0.3cm of replacement.east, anchor=west] {FORK};
\node[test=governcolor, right=0.3cm of constraint.east, anchor=west] {GOVERN};
\node[test=exitcolor, right=0.3cm of participation.east, anchor=west] {EXIT};
\node[test=codecolor, right=0.3cm of observability.east, anchor=west] (code) {CODE};
\node[test=auditcolor, right=0.15cm of code.east, anchor=west] {AUDIT};

% Failure descriptions
\node[font=\tiny\sffamily, text=gray, left=0.3cm of replacement.west, anchor=east, align=right] {Can system be\\reproduced?};
\node[font=\tiny\sffamily, text=gray, left=0.3cm of constraint.west, anchor=east, align=right] {Can rules be\\changed?};
\node[font=\tiny\sffamily, text=gray, left=0.3cm of participation.west, anchor=east, align=right] {Can users\\withdraw?};
\node[font=\tiny\sffamily, text=gray, left=0.3cm of observability.west, anchor=east, align=right] {Can errors be\\detected?};
...
\end{tikzpicture}
\caption{\textbf{Four Control Layers of Corrigibility.} Each layer addresses a distinct failure mode. Observability enables error detection; Participation enables error signaling; Constraint enables correction; Replacement ensures corrective capacity cannot be permanently blocked. Failure at any layer propagates upward.}
