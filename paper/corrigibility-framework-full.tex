...
% ============================================
% IEEE Two-Column Conference Style
% ============================================
...
\usepackage{mathtools} % Enhanced math display
...
Current policy discourse defines DPI through aspirational language. The G20 New Delhi Declaration (2023) and the UN/UNDP framework \cite{undp} describe systems that ``should be secure'' or ``can be built on open standards.'' These definitions exclude almost nothing. They articulate intent, not condition. Under such broad definitions, a system that traps users in a coercive loop can be labeled ``public infrastructure'' simply because it operates at scale.
...
\begin{quote}
``Digital Public Infrastructure (DPI) is a set of shared digital systems that should be secure, interoperable, built on open standards, enabling delivery of services at population scale'' \cite{g20}.
\end{quote}

The UNDP framework \cite{undp} elaborates:
...
\paragraph{Policy Frameworks.} The G20 framework \cite{g20} and UNDP definitions \cite{undp} establish aspirational principles (openness, inclusion, interoperability) without verification criteria. The World Bank's Identification for Development (ID4D) initiative emphasizes functional identity but does not specify architectural requirements for user exit or independent audit. These frameworks describe \textit{what} DPI should achieve without specifying \textit{how} to verify achievement.

\paragraph{Critical Analyses.} Scholarship on India Stack has documented exclusion harms \cite{khera2019} and constitutional tensions \cite{bhatia2019}. However, these critiques remain case-specific rather than providing generalizable tests applicable across jurisdictions and technologies.

\paragraph{Control Theory and Governance.} Cybernetic approaches to governance date to Ashby \cite{ashby1956} and Beer's viable systems model, but have not been systematically applied to digital infrastructure accountability. Lessig's ``code is law'' \cite{lessig2006} establishes that architecture constrains behavior, but does not specify which architectural properties are necessary for democratic legitimacy.

\paragraph{Software Freedom.} The free software tradition \cite{stallman2002} provides operational tests (inspect, modify, redistribute) but focuses on code artifacts rather than infrastructure systems that include data, compute, and network effects.
...
In cybernetics, the relationship between a controller and its environment is governed by Ashby's Law of Requisite Variety \cite{ashby1956}. The law states that for a system to be stable, the number of control states must meet or exceed the number of environmental states:
...
This dynamic was formalized by Hirschman \cite{hirschman1970} in his analysis of organizational decline. Hirschman distinguished two correction mechanisms: exit (withdrawal from participation) and voice (attempts to change the system from within). His central finding is that these mechanisms exist in productive tension, not as substitutes.
...
\begin{table}[p]
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{7cm}@{}}
            ...
            \midrule
            EXIT & $\times$ & Disproportionate penalty: exclusion from PDS, banking, SIM \cite{khera2019}. \\
...
The \textbf{Temporal Stability Condition} states: A system is corrigible only if the rate of effective correction ($C$) exceeds the rate of error accumulation ($E$) caused by environmental variety.
...
However, the timescale mismatch is a thermodynamic constraint, not a jurisdictional preference. A court operating on bureaucratic time ($\frac{dC}{dt} = k$) cannot mathematically regulate an automated system propagating errors at digital speed ($\frac{dE}{dt} \propto e^t$). The framework criticizes the \textit{absence} of native technical constraints, not the \textit{latency} of human justice.
\end{remark}
...
For practical robustness against measurement noise and burst errors, the framework defines a \textbf{robustness margin} $\delta > 0$:
...
When a system fails to correct errors due to the velocity mismatch described above, the demand for correction does not evaporate. It transforms. The principle of \textbf{Conservation of Correction Demand} states: for a given system state, the total pressure for correction is constant.
...
These dynamics have profound implications for constitutional review. Legal doctrines such as proportionality analysis, as established in \textit{Puttaswamy v. Union of India} \cite{puttaswamy2017}, presuppose that state intrusions can be measured, minimized, and remedied. However, as \citet{bhatia2019} argues, modern digital systems frequently violate the structural assumptions upon which these legal doctrines rest.
...
\subsection{The DPI Trilemma}
\label{subsec:trilemma}

The empirical pattern suggests a structural constraint governing state-backed infrastructure.

\begin{proposition}[The DPI Trilemma]
\label{prop:trilemma}
A state-operated Digital Public Infrastructure can strongly realize any two of the following three properties, but not all three simultaneously:
\begin{enumerate}[noitemsep]
    \item \textbf{Sovereignty}: local control over standards, execution, and data residency;
    \item \textbf{Scale}: population-level ubiquity and mandatory integration across essential services;
    \item \textbf{Neutrality}: non-discriminatory access and competitive fairness at the execution layer, guaranteed by structural corrigibility.
\end{enumerate}
\end{proposition}
...
\begin{tikzpicture}[scale=1.2, font=\sffamily]
    ...
    % Ashby annotation
    \node[font=\tiny, text=gray, align=center] at (0,-2.3) {Under Ashby's Law: centralization $\rightarrow$ reduced governance variety $\rightarrow$ Neutrality failure};
\end{tikzpicture}
\caption{\textbf{The DPI Trilemma.} States can achieve any two of Sovereignty, Scale, and Neutrality, but not all three. India Stack optimizes for Sovereignty + Scale at the cost of Neutrality. Federated systems (Linux, SMTP) achieve Neutrality + Scale without mandatory Sovereignty.}
\label{fig:trilemma}
\end{figure}

This is a control-theoretic constraint, not a policy preference. Achieving both Sovereignty (local control over the controller) and Scale (mandatory population coverage) requires centralized enforcement of standards and routing. That centralization reduces the variety of the governance channel: fewer independent actors can modify system behavior. Under Ashby's Law, reduced governance variety erodes Neutrality (the system's capacity to treat diverse inputs equitably). Conversely, systems that preserve Neutrality through distributed governance (Linux, Bitcoin, SMTP) typically lack mandatory scale or achieve it through federation rather than centralization.

India's experience instantiates this constraint: the architectural choices that achieved sovereign control and near-universal coverage produced structural dependency on a small set of switching and credentialing authorities. That dependency compressed the correction channel's bandwidth, manifesting as the GOVERN failures documented in Table~\ref{tab:govt_infra}. The Trilemma reframes these outcomes: the observed failures are not implementation mistakes but predictable consequences of optimizing for two properties at the expense of the third.
...
\paragraph{Definition.} An essential system $S$ satisfies FEE if compensatory architectural guarantees recreate the same effective error-signal strength that exit provides in non-essential systems.
...
where the weights $\alpha_i$ are calibrated via pilot audits. All variables are operationalized such that higher values indicate greater corrective pressure on the operator: a higher velocity of legal response, for instance, increases $E$, thereby satisfying the FEE condition. This formulation connects back to Ashby's Law: FEE is an engineered mechanism to maintain requisite variety in the controller when the natural market variety (EXIT) is suppressed.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[scale=0.9, font=\sffamily\small,
    block/.style={rectangle, rounded corners, draw=gray!70, fill=gray!10, minimum width=2.5cm, minimum height=0.8cm},
    arrow/.style={-{Stealth[length=2mm]}, thick}
]

% Non-essential system (top)
\node[anchor=west, font=\bfseries] at (-4.5, 3) {Non-Essential System:};
\node[block, fill=exitcolor!20, draw=exitcolor] (exit1) at (0, 2) {EXIT Available};
\node[block] (system1) at (4, 2) {System $S$};
\draw[arrow, exitcolor] (exit1) -- (system1) node[midway, above] {$E_{\text{exit}}$};
\node[font=\scriptsize, gray] at (2, 1.3) {Market departure = strong signal};

% Essential system without FEE (middle)
\node[anchor=west, font=\bfseries] at (-4.5, 0) {Essential System (No FEE):};
\node[block, fill=red!20, draw=red] (exit2) at (0, -1) {EXIT Blocked};
\node[block] (system2) at (4, -1) {System $S$};
\draw[arrow, red, dashed] (exit2) -- (system2) node[midway, above] {$E = 0$};
\node[font=\scriptsize, gray] at (2, -1.7) {No error signal = incorrigible};

% Essential system with FEE (bottom)
\node[anchor=west, font=\bfseries] at (-4.5, -3) {Essential System (with FEE):};
\node[block, fill=red!20, draw=red] (exit3) at (-1, -4) {EXIT Blocked};
\node[block, fill=governcolor!20, draw=governcolor] (fee) at (2, -4) {FEE Mechanisms};
\node[block] (system3) at (6, -4) {System $S$};
\draw[arrow, red, dashed] (exit3) -- (fee);
\draw[arrow, governcolor] (fee) -- (system3) node[midway, above] {$E_{\text{alt}} \geq \theta_E$};
\node[font=\scriptsize, gray] at (2, -4.8) {Compensatory signal = conditionally corrigible};

% FEE components
\node[font=\tiny, anchor=north, text width=3cm, align=center] at (2, -5.2) {
Multi-issuer +\\
Portability +\\
Fallback +\\
Federated
...
\end{tikzpicture}
\caption{\textbf{Functional Exit Equivalence (FEE).} Top: Non-essential systems generate error signals through market exit. Middle: Essential systems without FEE block error signals entirely. Bottom: Essential systems with FEE use compensatory mechanisms to restore error signal strength.}
\label{fig:fee}
\end{figure}
...
This section serves as a \textbf{stress test} of the corrigibility framework. If the five tests are truly fundamental, they must apply not only to deterministic infrastructure but also to stochastic, learned systems where the assumptions underlying traditional verification collapse entirely.
...
Therefore, publishing inference code alone does not satisfy the CODE test. It allows an auditor to run the machine, but not to understand why it produces specific outputs. To bridge this gap, compliance requires structured disclosure across the training pipeline, such as \textbf{Data Sheets for Datasets} \cite{gebru2021} and \textbf{Model Cards} \cite{mitchell2019}. These artifacts function as the ``source code'' for structural evaluation; withholding them renders the system a black box.
...
Let $\Delta O$ represent the \textbf{ontological contestability} of a system---the degree to which affected parties can challenge, modify, or replace the representation schema. RCR occurs when:
...
This extends the CODE test for world models: compliance requires not merely weight publication but \textbf{representation schema disclosure}---machine-readable specifications of the ontological categories the model employs, with documented justification for category boundaries.
...
This theorem formalizes what ``open-weights AI'' obscures: releasing weights while hoarding compute is \textbf{inference forkability without training forkability}. If OpenAI or Meta releases weights but $C_{\text{train}}$ exceeds 100$\times$ the capacity of global academic compute, $\text{FORK}_{\text{training}} = 0$.
...
where $\mathbf{1}[\cdot]$ is the indicator function and \texttt{data\_manifest\_available} requires Data Sheets for Datasets \cite{gebru2021} with full provenance.

\subsubsection{Compute as Governance Infrastructure}

The Compute Capture Theorem implies that \textbf{compute is itself DPI}. Legal forkability is insufficient absent practical compute accessibility. This motivates several architectural responses:

\begin{enumerate}[noitemsep]
    \item \textbf{Public Compute Nodes}: State-sponsored clusters with governance boards (academia + civil society + government), prioritized quotas for public interest retrains and independent audits. These function as research infrastructure analogous to national laboratories.

    \item \textbf{Mandatory Checkpoint \& Cost Disclosure}: Require publication of FLOP estimates, cost-of-train, training checkpoints (or intermediates), and data manifests with privacy protections. These enable evaluation of $C_{\text{train}}$.

    \item \textbf{Federated Training Pools}: Require DPI models affecting essential services to be trained via federated consortiums where multiple independent nodes share shards; no single node holds all data.

    \item \textbf{Compute Vouchers}: Grants for accredited auditors and civic labs to attempt reproduction, establishing practical $C_{\text{accessible}}$.
\end{enumerate}

The accessibility multiplier $\kappa$ provides regulators a concrete policy lever: if the ratio $C_{\text{train}} / C_{\text{accessible}}$ exceeds the threshold, additional mitigation is required (e.g., mandated lighter-weight models, specialized models, or co-training access).
...
Training data manifest & Required & Data Sheets \cite{gebru2021} with provenance \\
...
To measure the risk of \textbf{Epistemic Delegation}, the framework proposes the \textbf{Workflow Capture Coefficient (WCC)}:
\begin{equation}
WCC = f(C_{\text{workflow}}, O_{\text{substitutability}}, M_{\text{portability}})
\end{equation}

Where $C_{\text{workflow}}$ is the cost of reproducing the orchestration stack and $O_{\text{substitutability}}$ is the ease of altering the underlying ontological schema. A high WCC triggers a failure of the \textbf{FORK} test, regardless of whether the underlying model weights are open. \textbf{Open weights with closed workflows is performative transparency.}

\subsubsection{Fallout Governance: The Velocity Trap Realized}

When decision throughput ($\lambda$) and AI propagation rates ($E_{\text{AI}}$) outrun human legislative correction velocity ($C_{\text{policy}}$), the state enters a condition of \textbf{Fallout Governance}:
\begin{equation}
E_{\text{AI}} \gg C_{\text{policy}}
\end{equation}

In this state, policy ceases to guide behavior in advance; it instead reacts post-hoc to public scandal, litigation, or failure clusters. Governance is no longer anticipatory but becomes an exercise in damage containment. This is the Collingridge Dilemma realized at civilizational scale.
...
This reframing matters for policy. The discourse of ``AI risk'' often conflates capability with capture. Corrigibility separates these concerns: a system can be simultaneously powerful and accountable, or weak and authoritarian. The question is not ``How smart is it?'' but ``Can those it affects correct it?''

\begin{remark}[Necessary but Not Sufficient]
Corrigibility is a necessary, but not sufficient, condition for a good society. It does not guarantee fairness, eliminate bias, or produce just outcomes. It merely preserves the \textit{structural possibility} of contestation. A corrigible system can still encode unjust policies---but those policies can be challenged, measured, and changed. An incorrigible system forecloses even that possibility.
\end{remark}

\subsection{The Roadmap Out}

The structure of the five tests implies an ordering for transition: certain tests are prerequisites for others, and certain interventions have higher leverage.
...
\begin{enumerate}
    ...
    \item \textbf{AUDIT before GOVERN.} Verified error data creates political pressure for governance reform. Governance without measurement is arbitrary. The sensor must function before the actuator can be calibrated.
...
Corrigibility is the structural response to the Collingridge Dilemma: by the time the harms of an incorrigible system become politically undeniable, the system is too technically entrenched to dismantle.
...
\begin{enumerate}
    \item \textbf{Empirical measurement of correction velocity.} The velocity inequality (Section~\ref{subsec:velocity}) predicts system instability when correction latency exceeds error accumulation rate. Historical case studies (PDS exclusion incidents, payment system outages, identity fraud waves) could quantify the threshold at which systems become ungovernable.
    ...
    \item \textbf{AI-specific verification protocols.} The CODE test for learned systems (Section~\ref{sec:learned}) requires layered transparency across the training pipeline. Developing standardized, machine-verifiable disclosure formats would enable systematic AI corrigibility assessment.
    ...
    \item \textbf{Agentic system requirements.} Section~\ref{subsec:gdos} sketches how AI agents require corrigible infrastructure. Specifying precise API contracts for agent-compatible infrastructure would translate corrigibility requirements into engineering specifications.
\end{enumerate}
