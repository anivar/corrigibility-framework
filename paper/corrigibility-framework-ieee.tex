...
\begin{enumerate}[noitemsep]
    \item \textbf{Formal Framework}: This paper formalizes DPI corrigibility through five structural tests (EXIT, CODE, AUDIT, GOVERN, FORK) derived from cybernetic control theory, commons governance, and free software principles.
    \item \textbf{Stability Proof}: A control-theoretic proof demonstrates that partial corrigibility is equivalent to open-loop instability, explaining why systems that fail any single test accumulate uncorrectable harm.
    \item \textbf{Empirical Application}: The framework is applied to national identity systems (Aadhaar), payment infrastructure (UPI), and learned systems (AI), demonstrating systematic evaluation of real-world DPI.
    \item \textbf{Extension to AI}: The framework extends to stochastic learned systems, identifying the Temporal Variety Gap as a fundamental challenge and proposing verification methods for non-deterministic infrastructure.
    \item \textbf{Machine-Readable Schema}: JSON schemas enable automated corrigibility assessment and standardized infrastructure disclosure.
\end{enumerate}
...
\begin{clarification}[title=The Definitional Problem]
Current DPI policy specifies \textbf{intent, not conditions}. It articulates what systems \textit{should} achieve without specifying what they \textit{must} structurally provide. Under such definitions, a system that traps users in a coercive loop can be labeled ``public infrastructure'' simply because it operates at scale.

The framework proposed in this paper converts aspirational adjectives into verifiable predicates. It asks not ``Is this system described as open?'' but ``Can independent parties inspect its operation?'' Not ``Is this system intended to be accountable?'' but ``Can those it affects trigger correction?''
\end{clarification}
...
\label{def:govern}
A system $S$ satisfies \textbf{Constitutive Constraint} if there exists a governance function $G: \text{Rules} \to \text{Rules}'$ such that (1) $G$ is accessible to affected parties, not solely the operator; (2) $G$ is binding---the operator cannot override $G$'s output; and (3) there exists a cryptographic or legal chain of custody linking system behavior to $G$'s constraints.
\end{definition}
...
\paragraph{Switching Cost ($\sigma$).} We operationalize the practical barrier to exit and fork through a \textbf{switching cost} scalar:
...
\paragraph{Worked Example: Aadhaar vs UPI.} Consider switching from Aadhaar-linked services to hypothetical alternatives:
...
\toprule
\textbf{System} & $c_{\text{data}}$ & $c_{\text{down}}$ & $c_{\text{reprov}}$ & $c_{\text{net}}$ & $c_{\text{legal}}$ & $\sigma$ \\
\midrule
Aadhaar $\to$ Alt ID & 1.0 & 1.0 & 1.0 & 0.8 & 1.0 & 4.8 \\
UPI $\to$ Cash/Card & 0.2 & 0.1 & 0.3 & 0.4 & 0.0 & 1.0 \\
UPI $\to$ Alt Payment & 0.3 & 0.2 & 0.4 & 0.5 & 0.1 & 1.5 \\
\bottomrule
\end{tabular}
\caption{Illustrative switching cost estimates. Aadhaar exhibits near-maximal $\sigma$ because no data export exists ($c_{\text{data}} = 1$), service denial during transition is total ($c_{\text{down}} = 1$), and legal mandate forecloses alternatives ($c_{\text{legal}} = 1$). UPI retains lower $\sigma$ because cash and cards remain legal alternatives.}
\label{tab:switching-example}
\end{table}

With $\Sigma^* = 2.0$ as a policy threshold, Aadhaar ($\sigma = 4.8$) categorically fails EXIT/FORK while UPI ($\sigma = 1.0$--$1.5$) passes conditionally. The calculation reveals that Aadhaar's incorrigibility is not merely a governance failure but a mathematical certainty given its architectural constraints.

\paragraph{The Portability Threshold.} EXIT and FORK functionally fail when switching cost to all plausible alternatives exceeds a policy threshold:
...
\paragraph{Technical Requirements for Portability.} To ensure $\sigma < \Sigma^*$, DPI systems must implement:
\begin{enumerate}[noitemsep]
    \item \textbf{Standardized export bundle}: user-owned, signed, encrypted archive containing identity claims, transaction history, and verifiable credentials (format: JSON-LD with signed manifests)
    \item \textbf{Interoperability API}: ``ingest'' endpoint specification to accept ported bundles with defined mapping rules and time-to-accept guarantees (e.g., 72 hours)
    \item \textbf{Statutory acceptance guarantee}: regulation requiring public services to accept verified ported credentials if cryptographic lineage checks pass
\end{enumerate}

This is the lesson of PSD2 (Payment Services Directive 2) in banking: portability mandates reduced $c_{\text{data}}$ and $c_{\text{legal}}$, making bank-switching economically viable. Similar mandates are required for identity and data infrastructure.
...
\section{Control-Theoretic Formalization}
\label{sec:formalization}

To formalize the assertion that partial corrigibility is a fatal structural failure rather than a mere degradation in quality, we model Digital Public Infrastructure as a standard feedback control system subject to environmental disturbance. The five tests (EXIT, CODE, AUDIT, GOVERN, FORK) are not independent normative pillars; they form a sequential, interdependent cybernetic loop.

\subsection{Proof of Open-Loop Instability}
...
\textit{Justification}: Each test corresponds to a necessary component of the feedback loop. Partial compliance does not preserve loop closure. This follows directly from Theorem~\ref{thm:null-feedback}: since $L(s) = K(s) \cdot P(s) \cdot H(s)$, if any multiplicative term is zero, the entire loop gain vanishes.
...
For the system to remain stable not just asymptotically but locally (preventing irreversible harm accumulation), the correction rate must dominate the error rate. We formalize this with a friction coefficient.

\begin{condition}[Strict Dynamic Stability]
\label{cond:strict-friction}
Let $\epsilon > 0$ represent the ``friction cost'' of correction (e.g., legal latency, systemic resistance). A system is robustly stable if and only if:
\begin{equation}
\left| \frac{d}{dt} C(t) \right| \ge \left| \frac{d}{dt} E(t) \right| + \epsilon
\end{equation}
\end{condition}

This inequality dictates that post-hoc administrative remedies (e.g., courts or ombudsmen) are mathematically insufficient for infrastructure governance. Bureaucratic correction operates at a linear or constant rate ($\frac{dC}{dt} = k$), while digital errors, cascading failures, and agentic interactions propagate at exponential or high-frequency rates ($\frac{dE}{dt} \propto e^{t}$). Since there is no timescale $t$ for which a linear function dominates an exponential one, relying exclusively on external administrative checks guarantees eventual system collapse. The correction logic must be intrinsic to the technical loop.
...
\begin{clarification}[title=The Core Claim]
...
\label{eq:velocity-lower}
\end{equation}

However, correction velocity has both a \textbf{lower bound} (preventing harm accumulation) and an \textbf{upper bound} (preventing manipulation):

\begin{equation}
v_{\min} < v_{\text{correction}} < v_{\max}
\label{eq:velocity-bounds}
\end{equation}

\begin{clarification}[title=Velocity Bounds]
Constitutional democracy \textit{invented} latency: bicameralism, judicial review, amendment supermajorities. Friction is not failure; friction is how minorities survive majorities. The gap between accusation and punishment is where due process lives. \textbf{Instantaneous correction is lynching.}

The framework does not argue for automated, instant, or algorithmic governance. It argues for \textbf{availability of correction channels, not their speed}. Nothing in EXIT, GOVERN, or AUDIT requires immediacy. They require possibility without permission. Courts are a designed correction channel. They satisfy GOVERN (binding authority), AUDIT (independent review), and EXIT (injunctions prevent harm). \textbf{The framework criticizes absence, not latency.}
\end{clarification}
...
\subsubsection{Integral Stability and Robustness Margin}

The instantaneous condition $C(t) > E(t)$ is necessary but insufficient. A system may satisfy this condition momentarily while accumulating net harm over time. The \textbf{integral stability condition} captures persistence:

\begin{equation}
\int_{t_0}^{t_1} \left( C(t) - E(t) \right) dt \geq 0 \quad \forall \, t_1 > t_0
\label{eq:integral-stability}
\end{equation}
...
For practical robustness against measurement noise and burst errors, we define a \textbf{robustness margin} $\delta > 0$:

\begin{equation}
C(t) - E(t) > \delta \quad \text{(robust stability)}
\label{eq:robustness-margin}
\end{equation}

Systems operating at $C(t) \approx E(t)$ are marginally stable; any perturbation tips them into harm accumulation. The margin $\delta$ represents excess correction capacity that absorbs shocks. Empirical calibration of $\delta$ requires measuring error and correction rates during incident windows (see Future Work).
...
\subsubsection{Active Deception as Sensor Attack}
\label{subsec:active-deception}

A severe pathology occurs when systems actively misrepresent their state: \textbf{safety-washing} (compliance claims without verifiable properties), \textbf{open-washing} (nominal openness without structural rights), and \textbf{audit theatre} (inspection access to non-representative components). Under the cybernetic model, these constitute \textbf{sensor attacks}: deliberate corruption of the AUDIT channel that induces false confidence and accelerates divergence.
...
\section{Extension to AI and Agentic Systems}
\label{sec:learned}

The five tests extend to AI and autonomous agents with modified evidentiary standards. The key insight: \textbf{the tests remain invariant; only the verification method changes}.
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Test} & \textbf{Traditional Software} & \textbf{Learned Systems} \\
\midrule
EXIT & Can refuse the system & Disclosure + non-AI alternative \\
CODE & Source code inspection & Training pipeline transparency \\
AUDIT & Logic inspection & Statistical bounds + monitoring \\
GOVERN & Maintainer process & Governance over objectives \\
FORK & Copy and deploy & Resource forkability (compute + data) \\
\bottomrule
\end{tabular}
\caption{Verification methods for deterministic vs. learned systems}
\end{table}

For AI, publishing inference code alone does not satisfy CODE; the behavioral logic is encoded in weights, training data, and RLHF preferences. FORK faces resource barriers: legal permission without compute access is meaningless.

\subsection{Agentic Scaling and Governance Denial of Service (GDoS)}
\label{subsec:gdos}

As Digital Public Infrastructure transitions to support the ``Agentic Economy,'' corrigibility moves from a policy preference to a strict engineering requirement. Autonomous agents lack the biological resilience to handle bureaucratic friction.

We distinguish \textbf{structural corrigibility} from \textbf{AI Alignment}. Alignment asks: \textit{Can the operator control the system?} Corrigibility asks: \textit{Can the affected subject correct the system?} For autonomous agents operating on public infrastructure, the latter is a prerequisite for interoperability.

\subsubsection{Corrigibility as an API Contract}
An agentic system requires the five corrigibility tests as functional primitives:
\begin{itemize}[noitemsep]
    \item \textbf{EXIT is Exception Handling}: To an agent, a mandatory system with no exit path and a blocked state appears as a logic deadlock.
    \item \textbf{CODE is Documentation}: Agents require inspectability of logic to form valid execution plans.
    \item \textbf{AUDIT is Observability}: An optimization agent cannot function without a reliable failure signal.
    \item \textbf{GOVERN is Constraint Discovery}: Advisory guidelines result in undefined behavior; agents require binding state constraints.
\end{itemize}

\subsubsection{The Human Friction Buffer and GDoS}
Incorrigible infrastructures may appear stable under human load because humans absorb friction through delay, compliance, and informal workarounds. Autonomous agents remove this buffering layer.

Consider an autonomous procurement agent interacting with a mandatory payment API. On encountering a compliance flag, the agent receives an opaque error code. Because no machine-readable error taxonomy exists (Failed CODE) and dispute resolution is human-mediated (Failed GOVERN), the agent retries. Fleets of such agents will rapidly saturate the queue.

We define this failure mode as \textbf{Governance Denial of Service (GDoS)}. As agentic interaction scales, the volume, speed, and strategic diversity of agent actions mathematically exceed the system's centralized capacity to interpret and correct behavior. The human friction buffer is not a feature; it is a hidden subsidy that masks architectural incorrigibility.

\begin{condition}[Agentic Stability]
For an autonomous agent $\mathcal{A}$ to operate reliably within infrastructure $\mathcal{I}$:
\begin{enumerate}[noitemsep]
    \item \textbf{Reversibility:} $\forall$ state $s_t$, $\exists$ transition via EXIT such that $\text{Cost}(s_{t+1}) < \infty$ (no deadlocks)
    \item \textbf{Decidability:} $\forall$ input $x$, $\text{Compute}(x)$ is inspectable via CODE (no hidden constraints)
\end{enumerate}
...
Systems lacking distributed audit and rapid, binding governance lack the requisite variety to remain stable under agentic load. Infrastructure designed for human-speed governance will experience systematic breakdown under machine-speed operation.
...
\begin{table}[htbp]
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{8.5cm}@{}}
            ...
            GOVERN & $\times$ & No binding constraint: banks own NPCI, regulate themselves. \\
            FORK & $\times$ & No economic means: hub-and-spoke requires NPCI cooperation. \\
            \bottomrule
        \end{tabular}
        \vspace{0.3cm}
    \end{subtable}
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{8.5cm}@{}}
            ...
            GOVERN & $\times$ & No binding constraint: BCB sets and enforces Pix rules unilaterally. \\
            FORK & $\times$ & No reproduction means: central switch requires BCB; no independent implementation. \\
            \bottomrule
        \end{tabular}
        \vspace{0.3cm}
    \end{subtable}
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{8.5cm}@{}}
            ...
            \textbf{Test} & \textbf{Result} & \textbf{Evidence} \\
            \midrule
            EXIT & $\times$ & Disproportionate penalty: exclusion from government services. \\
            CODE & $\checkmark$ & Execution logic inspectable: MIT source on GitHub. \\
            AUDIT & $\checkmark$ & External testing possible: transaction logs, documented APIs. \\
            GOVERN & $\checkmark$ & Binding constraint: NIIS treaty, MIT license irrevocable. \\
            FORK & $\checkmark$ & Legal + technical + economic: 25+ country deployments prove feasibility. \\
            \bottomrule
        \end{tabular}
        \vspace{0.3cm}
    \end{subtable}
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{8.5cm}@{}}
            ...
            CODE & $\times$ & Execution logic not inspectable: bank implementations proprietary. \\
            AUDIT & $\checkmark$ & External testing: FCA oversight, TPP certification. \\
            GOVERN & $\checkmark$ & Binding constraint: CMA order, PSD2 regulation. \\
...
\begin{table}[htbp]
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{8.5cm}@{}}
            ...
            \textbf{Test} & \textbf{Result} & \textbf{Evidence} \\
            \midrule
            EXIT & $\checkmark$ & No disproportionate penalty: can run locally, switch models freely. \\
            ...
            GOVERN & $\times$ & No binding constraint: firm sets safety/capability trade-offs unilaterally. \\
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{8.5cm}@{}}
            ...
            GOVERN & $\times$ & No binding constraint: Google sets roadmap and certification unilaterally. \\
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{8.5cm}@{}}
            ...
            \textbf{Test} & \textbf{Result} & \textbf{Evidence} \\
            \midrule
            EXIT & $\checkmark$ & No disproportionate penalty: can switch to alternatives freely. \\
            CODE & $\checkmark$ & Full inspection: client and server source available (GPL/AGPL). \\
            AUDIT & $\checkmark$ & External testing: cryptographic properties independently verifiable. \\
            GOVERN & $\times$ & No binding constraint: Foundation board sets direction unilaterally. \\
            FORK & $\times$ & No economic means: code forkable but no federation locks network effects. \\
            \bottomrule
        \end{tabular}
    \end{subtable}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{DID-Washing}: Deploying decentralized identifiers without decentralized governance. Technical decentralization at the identifier layer with governance centralization at the trust layer is structural theater.
\end{enumerate}
...
The contrast is instructive. Aadhaar and UPI, promoted as model DPI, fail all tests. The eighteen systems in Table~\ref{tab:positive}, rarely described as ``DPI,'' pass all tests.
...
Systems that pass all five tests are disproportionately non-essential infrastructure. This reflects a structural tension:
...
When refusal means exclusion from food, shelter, or identity, exit ceases to function as feedback. The systems in Table~\ref{tab:positive} avoid this trap because alternatives exist; Aadhaar has been made prerequisite to existence.

\paragraph{Functional Exit Equivalence.} Essential services can achieve conditional corrigibility through compensatory mechanisms: protected non-digital alternatives, multi-issuer requirements, credential acceptance diversity, and federated implementation. Estonian X-Road demonstrates this approach: open protocol, distributed implementations, multi-stakeholder governance. Full treatment of FEE appears in the extended version.
...
For diagnostic purposes, gradation remains useful. Tables~\ref{tab:operational-proxies-main} and~\ref{tab:three-indicator-main} specify continuous measurements such as \texttt{penalty\_ratio}, \texttt{visibility}, and \texttt{fix\_latency\_observed}. The binary determination is the output; the continuous measurements are the inputs. Evaluators can examine the inputs to prioritize interventions even when the output is uniformly ``fail.''
...
\toprule
\textbf{Test} & \textbf{Continuous Score} & \textbf{Binary Flag} & \textbf{Threshold Metric} \\
\midrule
EXIT & \texttt{penalty\_ratio}: cost(exit) / cost(stay) & \texttt{legal\_barrier}: is exit criminalized? & Pass if $p \leq 1.2$ AND \texttt{legal\_barrier} = false \\
CODE & \texttt{visibility}: public LOC / total critical path LOC & \texttt{build\_reproducible}: deterministic build exists? & Pass if $v \geq 0.95$ AND \texttt{build} = true \\
AUDIT & \texttt{observability}: \% transactions independently verifiable & \texttt{access\_permitted}: third parties can test? & Pass if $o \geq 0.9$ AND \texttt{access} = true \\
GOVERN & \texttt{override\_resistance}: historical rate of operator overrules & \texttt{binding\_charter}: irrevocable legal instrument? & Pass if $r = 0$ AND \texttt{charter} = true \\
FORK & \texttt{cost\_ratio}: fork cost / community resources & \texttt{legal\_rights}: permissive license? & Pass if $c \leq 0.001$ AND \texttt{rights} = true \\
\bottomrule
\end{tabular}
\end{table}
...
\label{subsec:political-economy}
...
\subsubsection{The Limits of Administrative Redress}

A critical distinction separates political correction (elections, courts, amendments) from structural correction (EXIT, FORK). States are corrected politically; infrastructure sits \textit{below} politics. When DPI is captured by executive authority---as when Aadhaar operated for seven years without Parliamentary authorization---political correction mechanisms cannot reach it. Post-execution administrative remedies (courts, ombudsmen) operate on a slower timescale than digital execution, rendering them structurally insufficient. Corrigibility is fundamentally distributional: incorrigibility harms the marginal user who lacks voice because EXIT is blocked.
...
\begin{quote}
\textbf{Concession:} Identity, money, and law may be \textbf{inherently unforkable} coordination goods. The paper does not demand \textit{actual forking}; it demands \textit{credible threat of forking}. But for coordination goods, even the threat may undermine function.
\end{quote}

This is a genuine tension the framework cannot fully resolve. The stable equilibrium may be: latent forkability + strong coordination incentives. Email works not because everyone forks SMTP, but because anyone \textit{could}. Whether this equilibrium is achievable for identity infrastructure remains an open question.

\subsubsection{Utility Does Not Equal Accountability}
\label{subsec:utility}

The framework measures \textbf{accountability structure}, not \textbf{utility}. A system can score 0/5 and still be beneficial; the score indicates \textit{structural risk}, not \textit{current harm}. UPI has democratized payments, but the question is: \textbf{when UPI fails someone, can they correct it?} A house built without fire exits can function perfectly for decades, until there is a fire.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Assessment Skills}: \texttt{skills/assess/} --- Rules and test definitions
...
The schema repository is versioned independently from the theoretical papers to allow schema evolution as DPI systems mature.
...
\section{Extended Control-Theoretic Analysis}
\label{appendix:proofs}

This appendix provides extended analysis building on the Null-Feedback Instability Theorem (Theorem~\ref{thm:null-feedback}) established in Section~\ref{sec:formalization}. The following subsections develop supplementary formalizations for correction velocity dynamics, test mappings to control components, and agentic interoperability requirements.
...
\section{Schema Specifications (v2.0)}
\label{appendix:schema}

The normative schemas are maintained in a separate repository to enable independent evolution:

\begin{center}
\url{https://github.com/anivar/corrigibility-schema}
\end{center}

This appendix provides a conceptual overview. For implementation, use the canonical schemas from the repository.

\subsection{Infrastructure Manifest (Operator Disclosure)}

The \texttt{infrastructure.json} schema forces operators to make definitive commitments:

\begin{itemize}[noitemsep]
    \item \textbf{layers}: Architectural layers for decomposed evaluation (per Section~\ref{subsec:layers})
    \item \textbf{exit}: EXIT test details---data portability, deletion, alternatives, penalties
    \item \textbf{governance}: GOVERN test---grievance channels vs binding mechanisms
    \item \textbf{fork}: FORK test---legally permitted, artifacts available, constructed barriers
    \item \textbf{escalation}: Human oversight linkage with trigger conditions
    \item \textbf{corrigibility\_claim}: Boolean assertions for all five tests
    \item \textbf{operator\_signature}: Cryptographic commitment
\end{itemize}

\paragraph{Canonical Schema:} \url{https://github.com/anivar/corrigibility-schema/blob/main/schema/dpi/infrastructure.json}

\subsection{Audit Assessment (Auditor Finding)}

The \texttt{audit.json} schema replaces subjective grading with hard observational measurements:

\begin{itemize}[noitemsep]
    \item \textbf{results}: Boolean pass/fail for all five tests
    \item \textbf{verification\_tier}: Depth of verification (presence, behavior, proof)
    \item \textbf{layer\_verification}: Per-layer spec vs implementation gap analysis
    \item \textbf{governance\_verification}: Distinguishes grievance from binding governance
    \item \textbf{fork\_verification}: Constructed barriers, reproduction attempts
    \item \textbf{valid\_until}: Audit expiration (max 12 months)
    \item \textbf{contestation}: Status if operator disputes findings
    \item \textbf{revocation}: If auditor revokes the audit
    \item \textbf{auditor\_signature}: Cryptographic attestation
\end{itemize}

\paragraph{Canonical Schema:} \url{https://github.com/anivar/corrigibility-schema/blob/main/schema/dpi/audit.json}
...
\item[\textbf{Version 2.0 (Separation)}]
\textbf{Separated Schemas from Papers.} Moved normative schemas to independent repository (\url{https://github.com/anivar/corrigibility-schema}) to enable schema evolution independent of paper publication cycles. Added: verification tiers (presence/behavior/proof), audit validity periods (12-month max), contestation and revocation mechanisms, observatory model for delegated verification, human escalation triggers, FORK verification fields (constructed vs natural barriers). Extended for EPI: LWD-R structure, tiered adoption, action boundaries, variety drift monitoring, compute capture assessment, swapability metrics.
...
\textbf{Response:} The proposition identifies a structural constraint, not a reason to abandon analysis. Architectural responses include protected alternatives (guarantee non-digital equivalents) and federated implementation (no single provider essential). X-Road demonstrates 4/5 at population scale. The finding that fully corrigible essential services require architectural innovation is the framework's central contribution. Full treatment of Functional Exit Equivalence (FEE) appears in the extended version.
...
``You admit compute costs make AI retraining impractical. If FORK always fails for frontier AI, your framework just says 'all AI is incorrigible'---trivially true and useless.''

\textbf{Response:} The key distinction is \textit{inference forkability} (running weights) vs \textit{training forkability} (reproducing the model). Only training forkability satisfies FORK for governance purposes. Evidence requirements include published training code, data manifests, and compute cost estimates. The claim is not that all AI fails FORK, but that systems withholding these artifacts fail. OLMo, Pythia, and academic models demonstrate that FORK-compliant AI is possible. Full AI forkability treatment appears in the extended version.
