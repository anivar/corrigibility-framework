...
\paragraph{Sensitivity Analysis.} Let continuous inputs be $\{p, v, \tau, b, r\}$ (penalty ratio, visibility, latency, bindingness, resource ratio) as defined in Table~\ref{tab:operational-proxies}. The binary determination function $D$ is:
...
\paragraph{Reproducibility Standard.} An assessment is reproducible if an independent auditor, given only the evidence artifacts listed above, reaches the same pass/fail determination. Disagreements must trace to (a) threshold calibration differences or (b) missing evidence, not to ambiguity in the test definitions themselves.

\subsection{Three-Indicator Protocol}
\label{subsec:three-indicator}

For maximum reproducibility, each test uses exactly three indicators: one continuous score, one binary flag, and one threshold-based metric.

\begin{table}[htbp]
\centering
\small
\caption{Three-indicator protocol for reproducible assessment}
\label{tab:three-indicator}
