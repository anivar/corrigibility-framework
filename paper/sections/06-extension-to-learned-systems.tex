...
One potential mitigation is \textbf{federated compute pooling}: distributed networks of smaller compute providers that collectively offer training-scale resources without centralized control. This mirrors the federation model discussed for identity infrastructure (Section~\ref{subsec:essential}), where no single institution controls access to all services. Whether federated compute can achieve sufficient scale for frontier model training remains an open empirical question, but the architectural parallel is instructive: forkability may require not merely legal permission but distributed infrastructure capable of exercising that permission.