...
The response requires distinguishing two modes of correction. \textbf{Political corrigibility} operates through voice (elections, legislative debate, judicial review) and constitutes the mechanism by which \textit{states} are corrected. \textbf{Structural corrigibility} operates through exit and fork (the capacity to withdraw or reproduce) and constitutes the mechanism by which \textit{infrastructure} is corrected.

This distinction connects directly to the Collingridge Dilemma discussed in Section~\ref{subsec:velocity}. States are corrected politically, but political correction operates on constitutional time: legislative sessions, judicial proceedings, electoral cycles. DPI sits \textit{below} politics, operating on digital time. When infrastructure is captured by executive authority, political correction mechanisms cannot reach it before entrenchment makes change prohibitively costly. The Collingridge double-bind manifests precisely here: by the time democratic institutions recognize the need for correction, the infrastructure has become too embedded to modify.

Aadhaar exemplifies this gap. UIDAI operated for seven years on executive authority alone before Parliament passed enabling legislation in 2016, and then only as a money bill to circumvent Rajya Sabha scrutiny. Democracies possess constitutional correction mechanisms (elections, courts, amendments), but DPI often escapes their reach. The Constitution can be amended through Article 368; Aadhaar's architecture cannot be voted on at all.
...
The regulatory alternative has genuine merit. Regulators \textit{could} mandate error rate disclosure, demographic audits, and grievance service-level objectives. Political correction mechanisms (parliament, courts, electoral accountability) exist even without structural forkability. The speed-to-scale tradeoff is real, and development pragmatism deserves serious engagement rather than dismissal.

However, the regulatory alternative rests on assumptions that frequently fail in practice. First, ``regulate the loop'' assumes trustworthy regulators, but regulators typically report to the same executive branch that operates the infrastructure, raising the question of who regulates the regulator. Second, in captured states, ``regulate later'' functionally means ``never''; platform consolidation creates constituencies that block reform. Third, market dominance differs from regulatory monopoly: Gmail \textit{earned} users through features, and users \textit{can} leave for Proton, Fastmail, or self-hosted alternatives. By contrast, systems operating under \textit{regulatory monopoly} foreclose competition by law, not by merit. Gmail is incorrigible by choice; a legally mandated payment switch is incorrigible by law. Finally, the empirical record is discouraging: population-scale identity systems routinely operate for years before facing legislative scrutiny, and even after judicial review, meaningful independent audits remain elusive. The pattern repeats globally: deployment precedes accountability, and accountability never catches up.
