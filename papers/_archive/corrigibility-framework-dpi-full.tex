...
\begin{abstract}
Corrigibility is the minimal structural condition that keeps public systems reversible. This paper derives five architectural constraints necessary to bound systemic error in digital public infrastructure: inspectable logic (CODE), independent verifiability (AUDIT), reversible participation (EXIT), binding participatory constraint (GOVERN), and independent reproducibility (FORK). These constraints operate across observability, participation, constraint, and replacement layers. The absence of any single constraint converts infrastructure into an open-loop system. The framework provides testable criteria for evaluating real-world DPI systems and distinguishes structural corrigibility from transparency or advisory governance in isolation.
\end{abstract}
...
\paragraph{Core Thesis.} Public systems are either structurally reversible or structurally irreversible. Corrigibility marks that boundary.
...
\begin{itemize}
    ...
    \item \textbf{EPI Schemas}: \texttt{schema/epi/infrastructure.json}, \texttt{schema/epi/audit.json} (see companion paper)
\end{itemize}
