...
\renewcommand{\shorttitle}{Corrigibility as a Structural Precondition for DPI}
...
\begin{enumerate}[noitemsep]
    ...
    \item Independent verification (\textbf{AUDIT})
...
\begin{quote}
``Establish a trusted---unique, secure, and accurate---identity'' and ``[c]reate a responsive and interoperable platform,'' toward identification systems that are ``inclusive, trusted, accountable'' \citep{worldbank2021id4d}.
\end{quote}
...
\begin{tikzpicture}[
    ...
    % FORK as expansion
    \node[block, fill=forkcolor!20, dashed] (fork) at (-3.4, -1.8) {FORK};
    ...
    \draw[->] (audit) -- node[above=2pt, label] {Controller} (govern);
    \draw[->] (govern) -- (sum1);

    % FORK expansion
    \draw[->, dashed] (fork) -- node[below=2pt, label] {Variety Expansion} (govern);

    % Annotations
    \node[font=\scriptsize, text=red!70!black, align=center] at (3, -3.2) {Failure of any component $\rightarrow$ open-loop divergence};
\end{tikzpicture}
...
The framework does not equate auditability with direct participation. AUDIT $\neq$ ``everyone audits''; AUDIT $=$ ``no one can prevent auditing.'' \textbf{Journalism analogy:} Not everyone investigates corruption. Democracy still depends on the \textit{possibility} of investigation. The problem today is not that auditors do not exist; it is that operators can legally block them. The tradeoff is contestable expertise over unchallengeable authority, a defensible democratic position.
\end{remark}
...
\begin{itemize}
    ...
    \item \textbf{Constructed barriers}: NPCI is the \textit{only permitted} instant payment switch in India \citep{rbipso2026}. The barrier is regulatory, not economic. Even with infinite capital, you cannot legally compete with UPI's core switching function.
...
\draw[arrow] (cyber.south) -- ([xshift=-10pt]audit.north);
...
% Connections - FOSS
\draw[arrow] (foss.south) -- ([xshift=10pt]code.north);
...
\node[instrument] (i-govern) at (6,-1.3) {Rule A.9\\Evidence};
...
% Labels
\node[font=\footnotesize, gray, align=center] at (4,-2.3) {All five tests required. Failure of any one disqualifies.\\Determination is evaluated at the least-resourced stratum, in both the inward and outward exercise.};
\end{tikzpicture}
\caption{The framework at a glance. Three independent theoretical traditions (top) derive the five corrigibility tests (middle), and the tests in turn derive their verification instruments (bottom): Functional Exit Equivalence (Section~\ref{subsubsec:fee}), the open-washing taxonomy, verification tiers, the Rule~A.9 evidence standard (Appendix~\ref{appendix:rules}), and the constructed-versus-natural barrier taxonomy, each shown under the test it most directly serves. Determination is evaluated at the least-resourced stratum (Remark~\ref{rem:argmin}) and in both the inward and outward exercise of each test.}
\label{fig:framework}
\end{figure}
...
One deployed mechanism already exercises a test in the outward voice at population scale, and the concession is stated here because it sharpens the framework rather than weakening it. In the digital-wallet paradigm, credential presentation executes through the subject: the holder determines when, where, and how credentials are shared, and refusal-to-present is a subject-side act that operates ex ante, at machine speed, inside the execution loop \citep{worldbank2026wallets}. In-loop subject authority is therefore demonstrated as buildable at population scale for exactly one verb, disclose. No deployed system extends the same architectural position to contest, correct, revoke, or halt. The demonstration converts each remaining outward omission from a claim of technical infeasibility into an architectural choice, and the Post-Execution Fallacy (Section~\ref{subsec:post-execution}) should be read with this concession attached: the operator's objection that in-loop subject authority cannot be implemented at scale is no longer available.
...
\begin{table}[p]
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{7cm}@{}}
            ...
            \midrule
            EXIT & $\times$ & Disproportionate penalty: statutory PDS linkage; de facto banking/SIM insistence post-\textit{Puttaswamy~II} \citep{puttaswamy2018,khera2019}. \\
            CODE & $\times$ & Execution logic opaque: proprietary matcher, closed algorithms. \\
            AUDIT & $\times$ & External testing barred: error measurement prohibited without UIDAI authorization \citep{aadhaaract2016}. \\
    ...
    \begin{subtable}{1\textwidth}
        ...
        \begin{tabular}{@{}llp{7cm}@{}}
            ...
            FORK & $\checkmark$ & 20+ country deployments prove feasibility. \\
            \bottomrule
        \end{tabular}
    \end{subtable}
...
\rowcolor{green!8}
Bluesky (AT Protocol) & Bluesky PBC & \textbf{5/5} & MIT/Apache; portable DID identity and self-hostable PDS make the fork threat credible, which is what binds PBC direction-setting (contrast Signal's FORK fail: state non-portability, so no latent-fork discipline) \\
...
\rowcolor{green!8}
Apache HTTP & Apache Foundation & \textbf{5/5} & Apache 2.0 license, ASF governance, powers a substantial share of the web \\
...
\paragraph{Cash Elimination.} Cash-reduction advocacy sometimes invokes the carbon footprint of physical money. Whatever the environmental accounting, the governance effect is uncontested: elimination of cash creates mandatory digital payment infrastructure with no EXIT option.
...
The absence is not incidental to one framework. The World Bank's digital-wallet policy notes, the series specifying the coming decade's identity substrate, contain no subject-corrective vocabulary at all: redress, grievance, appeal, recourse, contestation, and correction appear nowhere in either architectural note, while issuer-side revocation appears eleven times in the principal architecture note alone \citep{worldbank2026wallets}. In the companion trust-framework note the corrective machinery runs between institutions throughout: dispute resolution and complaint handling address participant non-conformity, remedies reach the people a failure lands on only through a liability allocation, and the note's single engagement with a subject contesting anything is the clause that shifts the burden of proof onto the party contesting a signature \citep{worldbank2026trust}. Existing DPI frameworks specify governance for operators. They provide no subject-side corrective mechanism; the single subject-side act the wallet paradigm executes inside the loop is disclosure (Section~\ref{subsec:dual-exercise}), and no corrective verb accompanies it. GOVERN, as this framework defines it, is not a stricter version of an existing column; it is the column the existing frameworks do not have.
...
\begin{tikzpicture}[scale=1.2, font=\sffamily]
    ...
    % Ashby annotation
    \node[font=\scriptsize, text=gray, align=center] at (0,-2.3) {Under Ashby's Law: centralization $\rightarrow$ reduced governance variety $\rightarrow$ Neutrality risk};
\end{tikzpicture}
...
\paragraph{The Political-Theoretic (Legitimacy) Argument.} Partial contestation creates \textit{sham governance}: cosmetic accountability without corrective leverage. A system that admits 10\% of grievances, or that admits all grievances but resolves them only after harm has accumulated past reversibility, is institutionally distinct from a system with no contestation only in appearance. Beyond a threshold of feedback quality, additional gradations in compliance produce \textit{non-monotonic} legitimacy gains: increases in nominal contestation procedures may decrease real correction capacity by absorbing political pressure into procedure rather than outcome. Below this threshold, a system forfeits the right to claim public legitimacy under any non-domination criterion (Section~\ref{sec:theory}, Normative Anchor). The threshold is binary because legitimacy is binary: a prison that permits appeals 10\% of the time is still a prison. The binary is not stipulated. Under the non-domination criterion, domination consists in the standing possibility of arbitrary interference rather than in its exercise \citep{pettit1997}; legitimacy therefore tracks whether that possibility has been structurally foreclosed, and foreclosure is a property a system has or lacks. Degrees of benign behavior do not alter it, and an unverifiable constraint leaves the possibility standing.
...
GOVERN & \texttt{override\_rate}: operator overrule rate & \texttt{charter}: irrevocable instrument? & override rate $= 0$ AND charter = true \\
FORK & \texttt{cost\_ratio}: fork / community resources & \texttt{rights}: fork-permitting license? & $r \leq 0.001$ AND rights = true \\
\bottomrule
\end{tabular}
\end{table}
...
\node[circle, fill=white, draw=red, inner sep=1pt, cross] at (2, -2.75) {};
\node[below, font=\tiny, color=red] at (2, -2.9) {Blocked (Aadhaar)};
...
UPI \textit{has} expanded access to digital payments at population scale. The framework risks dismissing real-world impact if this is not acknowledged. But the question the framework asks is different: \textbf{When UPI fails someone, can they correct it?}
...
\begin{itemize}
    ...
    \item \textbf{Protocol Documentation}: \texttt{PROTOCOL.md}, \texttt{AGENTS.md}, \texttt{docs/} --- test and analysis documentation covering EXIT, CODE, AUDIT, GOVERN, FORK, layer decomposition, variety drift, and action boundaries; assessments emit \texttt{audit.json} against the repository's versioned protocol
...
\draw[arrow, red!70!black, dashed] (infra.south) -- (1.5,-0.5) -- ([xshift=-8pt]compare.north);
\draw[arrow, red!70!black, dashed] (corrig.south) -- (4.5,-0.5) -- ([xshift=8pt]compare.north);
\end{tikzpicture}
...
In either case, the effective corrective feedback is $u_{\text{fb}} = 0$.
The system evolution depends only on initial intent $u_{\text{open}}$ and disturbance:
\begin{equation}
y(t) = \int_0^t (u_{\text{open}}(\tau) + d(\tau)) d\tau
\end{equation}
...
\begin{tikzpicture}[node distance=1.8cm, auto, >=latex, thick]
    ...
    \node [below of=H, node distance=0.85cm] {\scriptsize AUDIT/EXIT};
...
\begin{tikzpicture}[node distance=1.8cm, auto, >=latex, thick]
    ...
    \node [below of=H, node distance=0.85cm, red] {\scriptsize AUDIT = 0: Sensor Blindness};
...
\begin{tikzpicture}[scale=0.9]
    ...
    \node[anchor=east] at (axis cs:4.9,6) {\scriptsize Instability};
    \end{axis}
\end{tikzpicture}
...
To enable third-party reproduction of pass/fail determinations, each test requires specific evidence types (Table~\ref{tab:evidence-requirements}).
...
\begin{enumerate}
    ...
    \item Discrete integrator supplies an elementary divergence demonstration and absorbs Condition~\ref{cond:friction} through its latency and efficiency-loss parameters ($\tau$, $\eta$), which correspond to the continuous friction cost $\epsilon$.
...
\begin{tikzpicture}[scale=0.85]
    ...
    \node[font=\scriptsize, text=red!70!black, anchor=west] at (axis cs:5.6,5.3) {$Sc^*$};
    \draw[red!70!black, dashed, thin] (axis cs:5.45,0) -- (axis cs:5.45,5.6);

    % Neutrality threshold line
    \addplot[red, thick, dotted] {0.88*x};
    \addlegendentry{$\theta \cdot V_{\text{env}}$ (threshold)}
    \end{axis}
\end{tikzpicture}
\caption{\textbf{Governance Variety Compression Under Scale.} As scale increases, environmental variety grows linearly. Under centralized enforcement, governance variety grows sublinearly (logarithmic here); $V_{\text{restored}}$ denotes the variety contributed by the compensatory mechanisms of Section~\ref{subsec:trilemma} (federated execution, multi-issuer mandates, FEE, state portability). Neutrality degradation risk emerges when governance variety falls below the threshold $\theta \cdot V_{\text{env}}$ (at $Sc^*$).}
