...
Digital Public Infrastructure (DPI) is evaluated using aspirational criteria: openness, interoperability, inclusion, scale. These descriptors do not answer a fundamental question: can the system be corrected when it goes wrong?
...
These constraints operate across participation, observability, governance, and replacement layers. The absence of any one constraint creates structural asymmetry. The framework provides testable criteria for evaluating real-world DPI systems.
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Control-Theoretic Formalization}: A mathematical proof (Appendix~\ref{appendix:proofs}) demonstrates that partial compliance with these tests is functionally equivalent to open-loop instability.
...
\paragraph{Collective-Standing Precondition (Ostrom Principle~7).} Ostrom's eighth design principle identifies nested enterprises; her seventh identifies the minimal recognition of rights to organize. Both are preconditions for the mechanism families through which GOVERN operates at population scale --- class-action vehicles, deliberative forums, and juridical standing all presuppose that affected parties can constitute themselves as a collective actor, with rights to associate, aggregate claims, and fund representation. The framework's GOVERN test verifies the channel; Principle~7 verifies whether the subject that uses the channel can be constituted. Where the right to organize is suppressed --- by association law, administrative harassment, cost barriers that foreclose legal aggregation, or structural atomization --- the formal availability of GOVERN mechanisms does not translate into operative corrective authority, and the channel is open-loop in fact even when structurally present. GOVERN determinations therefore carry the implicit precondition that the affected class's right to organize, aggregate claims, and fund representation is legally protected and historically exercised at the relevant stratum (Rule~A.9 evidence standard). Where this precondition fails, the audit must record GOVERN as conditionally open and specify the associational constraint. Individual grievance pathways are not substitutes; they address individual cases without generating the collective error signal that governance correction requires.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../shared/figures/topology-of-necessity.pdf}
...
\begin{table}[htbp]
    \centering
    \scriptsize
    \caption{\textbf{Evaluation of Private/Platform Infrastructure.} Technical openness (Source Code/APIs) does not guarantee structural accountability.}
...
\begin{equation}
E_{\text{alt}} \geq \theta \times E_{\text{baseline}}
\end{equation}
where $\theta$ is domain-specific (healthcare: 0.95; payments: 0.90; social benefits: 0.85). Example: if the Public Distribution System delivers food at $E_{\text{baseline}} = 0.80$ coverage, and $\theta = 0.90$, then any alternative must achieve $E_{\text{alt}} \geq 0.72$ to preserve functional exit. FEE operationalizes EXIT. Without it, ``opt-out'' is administrative theater.
...
\paragraph{Bole Clause: Evidentiary Standard for Statutory Fallbacks.} A statutory fallback satisfies FEE only if it meets the Rule~A.9 evidentiary standard applied at the margin stratum. Rule~A.9 requires: (i) a resolvable reference to the enabling instrument, (ii) its cryptographic hash, and (iii) historical evidence of enforcement. For statutory fallbacks the third requirement is operative availability at the margin: documented successful invocations of the analogue pathway by members of the least-resourced strata --- those with low digital literacy, remote location, missing biometric records, or administrative precarity --- within the audit window. A fallback legally guaranteed but practically inaccessible at the margin produces $E_{\text{alt}} \approx 0$ at $x^*$ regardless of its nominal coverage; it is operationally absent for the population whose error signals the framework's central evidence (Section~\ref{sec:diagnosis}) records as fatally suppressed. Mechanism~5 (Funded Fallback Operations) mitigates but does not close this gap without margin-stratum invocation records: funding is necessary, operative availability at the margin is the test.

The historical precedent is instructive. The Bole Resolution of 1923 was a legislative guarantee of access to public water sources in the Bombay Presidency. It remained a dead letter for four years until collective action enforced it at Mahad, then required a further decade of litigation to survive the local order's counterattack. A statutory fallback is a GOVERN-class claim; GOVERN-class claims require enforcement history by the relevant subject class, not legislative text alone \citep{ambedkar1947states}.
...
The political economy analysis above named the structural pressures that drive feedback-loop closure to fail. What remains is whether the binary determination holds under measurement uncertainty, whether the loop survives adversarial conditions, and what transition pathways remain open once a system is diagnosed as structurally incorrigible. This section takes those three threads in turn: the cybernetics of binary evaluation, the political economy of accountability, and the architectures of corrigible public infrastructure.
...
A system that allows \textbf{AUDIT} but blocks \textbf{GOVERN} is an \textbf{Open Loop}: it observes errors but cannot fix them. A system that allows \textbf{GOVERN} but blocks \textbf{EXIT} is a \textbf{Coercive Loop}: it traps energy until catastrophic failure. Figure~\ref{fig:failure_modes} contrasts these failure topologies against the closed-loop case. Partial compliance creates ``zombie infrastructure'': systems maintaining the aesthetic of responsiveness while severing actual stability capacity. A ``Partial'' score represents \textbf{Fatal Structural Failure}.
...
\centering
\small
\caption{Policy constants used throughout the framework. Values are recommended defaults and calibration starting points; they are separable from the structural argument and require deliberative governance to set and revise (see Section~\ref{subsec:limitations}). The constants are policy choices, not engineering discoveries: every threshold that defines a pass creates an operator interest in placing the pass just below their system's measured value. Calibration must therefore itself satisfy a deliberative-mechanism condition --- affected-class participation, documented rationale, and periodic external review --- on the same structural grounds that GOVERN cannot be operator self-certification. Per-test pass/fail thresholds (penalty ratio $p$, visibility $v$, observability $o$, governance throughput $\tau$, fork resource ratio $r$) are listed separately in Tables~\ref{tab:operational-proxies-main} and~\ref{tab:three-indicator-main}.}
...
$\Sigma^*$ & Portability threshold (EXIT/FORK failure) & e.g.\ $>0.2$ income or $>30$ days disruption & \ref{appendix:proofs} \\
...
\begin{enumerate}
    ...
    \item \textbf{Empirical scope limitations and the missing political-economy chapter.} The primary case studies (Aadhaar, UPI) derive from India Stack; systematic validation across other national DPI implementations (Brazil Pix, Singapore NDI, Kenya M-Pesa) is required before universal claims are established. The paper also does not formally analyze the political economy of why governments choose incorrigible designs when corrigible alternatives exist. Four mechanisms compose the explanation: (1) \textit{Graded inequality as demand-side driver} \citep{ambedkar1936annihilation} --- every administrative rung gains standing over the rung below from an incorrigible stack; the coalition for correction fractures by design, because the people who bear the cost of incorrigibility are precisely those whose exit is most constrained and whose collective action is most structurally opposed; (2) \textit{Artifact openness as legitimacy cover} --- releasing code or weights under an open license satisfies the form of the commons claim while retaining governance closure; ``open DPI'' is the joint electorate at infrastructure scale \citep{ambedkar1945congress}, offering the form of inclusion while the operator retains power over the form; (3) \textit{Administrative synopticism} --- states have structural preferences for downward legibility, and corrigible designs by definition introduce upward channels that constrain administrative discretion; the information asymmetry that incorrigibility produces is often an intended feature, not an oversight; (4) \textit{Certification capture} --- openness properties (open license, open standard) are certified as if they were governance properties (collective choice, binding accountability), and the certification layer is captured by parties whose deployments it evaluates. Together these four mechanisms predict that incorrigibility is the default equilibrium in essential-services infrastructure, not a correctable anomaly. This is not analyzed formally in the present paper; it is stated here as the political-economy chapter the framework implies and does not yet provide.
...
Corrigibility is a binary structural property. If any one of the five tests (EXIT, CODE, AUDIT, GOVERN, FORK) fails, systemic error cannot be structurally bounded, and corrective authority becomes discretionary rather than guaranteed.
\end{corollary}
...
\label{rem:argmin}
The Corollary is evaluated at the bottom of the gradient. Proposition~\ref{thm:null-feedback} treats $H$ and $K$ as system constants. In population-scale infrastructure, they are distributions over participant position $x$ in the social hierarchy: $H(x)$ is the probability that a grievance from position $x$ generates a registered error signal, and $K(x)$ is the probability that a registered error triggers binding correction visible to $x$. Both are strictly decreasing toward the least-resourced stratum --- affected by digital literacy, language, connectivity, time, administrative precarity, and fear of retaliation.

A system is corrigible if and only if the loop closes for all participants:
\begin{equation}
\text{Corrigible}(S) \iff \forall x \in \mathcal{X}: H(x) > 0 \;\land\; K(x) > 0
\end{equation}
The binding determination is taken at $x^* = \arg\min_x \bigl[ H(x) \cdot K(x) \bigr]$, not at the population mean. A system that closes the loop for median users while $H(x^*) = 0$ or $K(x^*) = 0$ at the margin satisfies the formal model for the comfortable majority and fails it entirely for those the system's own evidence (Section~\ref{sec:diagnosis}) shows are most harmed by incorrigibility.
...
\textbf{Implication:} Post-hoc remedies (Courts/Ombudsmen) have a linear or constant rate of operation ($\frac{dC}{dt} = k$). Digital errors propagate at exponential or high-frequency rates ($\frac{dE}{dt} \propto e^{t}$). Since there is no $t$ for which a linear function dominates an exponential one, relying on external courts guarantees eventual system collapse (Figure~\ref{fig:velocity-mismatch}). The correction logic must be intrinsic to the loop.
