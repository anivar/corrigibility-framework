...
\paragraph{Structural Pressures in Learned Systems.} Three additional pressures constrain corrigibility in learned systems. Table~\ref{tab:structural-pressures} maps each pressure to its architectural response.
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Pressure} & \textbf{What Breaks} & \textbf{What Fixes} & \textbf{Layer} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Three structural pressures in learned systems: what corrigibility condition they break, what architectural mechanism restores it, and which control layer is affected.}
\label{tab:structural-pressures}
\end{table}
...
Let $D$ be the disclosed representational schema and $S$ the deployed system. The R requirement of LWD-R is satisfied if and only if an independent auditor can reproduce the observed classification distribution of $S$ on a held-out probe set using only $D$ and the disclosed deployment configuration. If the deployment variables (quantization, routing, filtering) are not enumerated in $D$, the derivation chain breaks and the system fails the R requirement.
\end{proposition}
...
Training forkability requires more than compute access; it requires data sovereignty. Compute Capture and Data Capture act as independent multipliers of the Workflow Capture Coefficient: a system can satisfy one and fail the other and still be unforkable in practice. Discussions of ``open AI'' that focus exclusively on compute miss this second axis.
...
Training forkability is genuinely available if and only if a system evades both Compute Capture and Data Capture simultaneously:
\[
...
\paragraph{Alignment with OSI Definition.} The Open Source Initiative's Open Source AI Definition 1.0 \citep{osi2024} codifies this distinction, requiring: (1) data information sufficient for recreation, (2) complete training code, and (3) model parameters. By this standard, Llama 3.1/3.3, Gemma 2, DeepSeek-V3/R1, and Mistral do not qualify as open source AI despite marketing claims; OLMo 2 and Pythia do. The EU AI Act \citep{euaiact2024} similarly distinguishes between models with varying transparency obligations. This framework's FORK test operationalizes the same principle: without training forkability, governance correction is structurally impossible.
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Context-Window Isolation:} Constraints are encoded in the execution environment, not injected into the model's context window. A learned system cannot prompt-inject past constraints it cannot read; isolation is the structural answer to prompt-injection attacks.
...
If the state cannot govern the neural network's weights, it must govern the action boundary. The action boundary is the deterministic envelope that wraps stochastic inference; the model proposes, the validator disposes.
\end{principle}
...
The action boundary must therefore emit, for each rights-affecting determination, a \textbf{subject receipt}: a signed, structured record including the action class, the validator decision, the timestamp, and an inclusion proof against the audit log (transparency-log style, per RFC 9162 or equivalent). The receipt must be: (i) \textbf{subject-facing} --- delivered to the individual through a channel they can access offline, not only to the operator's logging infrastructure; (ii) \textbf{independently verifiable} --- the inclusion proof must allow the subject, or any party they designate, to confirm that the receipt corresponds to an entry in the canonical audit log; and (iii) \textbf{machine-readable} --- in a format that class-action aggregators and juridical mechanisms can ingest as evidence, because the juridical mechanism family (Section~\ref{subsec:govern-mechanisms}) depends on these receipts as the primary evidence that a systemic pattern exists.
...
Adopting an open action-specification architecture drives WCC toward zero, preserving interpretive sovereignty and satisfying the \textbf{FORK} test for the agentic era.
...
\begin{enumerate}[noitemsep]
    \item \textbf{Juridical.} Courts and statutory tribunals with jurisdiction to compel rectification, injunction, or damages. Juridical mechanisms attach when administrative determinations produce legally cognizable harms (denied benefits, criminal sanctions, immigration consequences). Their corrective force depends on standing rules, evidentiary admissibility for algorithmic records, and remedies that reach the operative system rather than the individual case. Because juridical remediation moves at bureaucratic speed against machine-time execution, public EPI must natively expose \textbf{Injunction Hooks} (Figure~\ref{fig:injunction-hook}): architectural APIs in the action boundary $B$ built to accept rapid, cryptographically signed halt or rollback flags issued by recognized courts, executed without waiting for operator compliance. Without such hooks, an injunction that takes weeks to arrive lands on a system whose accumulated determinations have already become irreversible, and the juridical mechanism degrades from corrective intervention to retrospective declaration.
...
\node[box, right=1.0cm of boundary] (exec) {Subject-class operations};
...
Table~\ref{tab:edge-bytecode} summarises the corrigibility impact of these architectural choices across the five tests.
...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{For technologists}: The Action Boundary Protocol offers a practical architecture for building corrigible AI systems. Deterministic validators wrapping stochastic inference satisfy GOVERN without requiring interpretability of model internals; multiple open protocols can instantiate the requirement.
...
\paragraph{Adversarial Robustness.} This framework assumes good-faith actors operating within institutional constraints. It does not address adversarial manipulation: coordinated gaming of audit mechanisms, governance capture through procedural compliance, prompt injection attacks on action boundaries, or fork-and-poison attacks on model provenance. Extension to adversarial settings requires additional machinery beyond this paper's scope.
...
The operator must declare exactly how the stochastic model is bounded. The \texttt{agentic\_workflow\_governance} object proves the system is using deterministic, externally specified action definitions rather than proprietary black-box orchestration.
