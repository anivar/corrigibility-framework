...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{R (Representation)}: The categorical schema (\textit{how the deployed system sees the world})
\end{itemize}
\end{definition}
...
The operator must declare exactly how the stochastic model is bounded (schema identifiers use the schema repository's URN namespace; sample fields are illustrative, the normative field set being versioned in the repository). The \texttt{agentic\_workflow\_governance} object proves the system is using deterministic, externally specified action definitions rather than proprietary black-box orchestration.
