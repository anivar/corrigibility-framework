...
\date{April 2026 \\ {\small Revised: 11 July 2026}}
...
\node[response] (r-govern) at (6.9,-1.4) {Action Boundary,\\scale bounds};
...
\end{tikzpicture}
\caption{Framework map. Learned systems add three structural pressures (top), each degrading a specific corrigibility test (middle): epistemic opacity breaks CODE at the observability layer, acceleration overwhelm breaks GOVERN at the constraint layer (restored by the Action Boundary together with fan-out, action-rate, and blast-radius bounds), and compute capture breaks FORK at the replacement layer. The architectural responses (bottom) restore verification for each test without softening the standard. Verification methods per test are detailed in Table~\ref{tab:verification}.}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Representation (R)}: The operative categorical schema (ontological categories and label schemas)
\end{itemize}
...
To bridge this gap, compliance requires structured disclosure across the training pipeline, such as \textbf{Datasheets for Datasets} \citep{gebru2021} and \textbf{Model Cards} \citep{mitchell2019}. These artifacts function as the ``source code'' for structural evaluation; withholding them renders the system a black box.
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{R (Representation)}: The categorical schema (\textit{how the deployed system sees the world}), in its operative form (Section~\ref{subsubsec:operative-r})
\end{itemize}
\end{definition}
...
The distinction matters because deployment-time variables fundamentally modify the representational boundary, often without altering the disclosed weights \citep{hooker2019}:
...
The firewall's criterion is disclosure sufficient for independent behavioral reproduction of the operative representation (Proposition~\ref{prop:operative-r-test}); it does not demand mechanistic interpretation of the latent geometry, which remains beyond inspection by design. The name marks where interpretability limits bite, namely adoption eligibility, not a claim that they can be solved.
...
\paragraph{The Gap Closes from Both Ends.} Variety Drift is the model-side half of the temporal problem. The controller side erodes too: skills exercised only through a system atrophy outside it, the long-documented irony of automation \citep{bainbridge1983}, so the governance variety available for correction decays endogenously even as environmental variety grows. The persistence condition is therefore conservative. Measuring controller-side erosion alongside model-side drift is an open problem.

\paragraph{The Revalidation Threshold.} When variety drift exceeds a domain-specific threshold $\tau_{\text{drift}}$, mandatory revalidation is triggered:
\begin{equation}
\Delta(t) > \tau_{\text{drift}} \implies \text{Revalidation Required}
\end{equation}
...
\begin{equation}
D_{\text{required}} > \kappa_D \cdot D_{\text{accessible}}
\end{equation}
...
\[
\text{FORK}_{\text{training}}^{\text{full}} \;=\; \mathbf{1}[C_{\text{train}} \leq \kappa \cdot C_{\text{accessible}}] \;\land\; \mathbf{1}[D_{\text{required}} \leq \kappa_D \cdot D_{\text{accessible}}].
\]
\end{proposition}
...
Training data manifest & Required & Datasheets \citep{gebru2021} with provenance \\
...
\paragraph{Concrete Examples.} As of mid-2026, the open-model landscape divides cleanly along this axis:
\begin{itemize}[noitemsep]
...
This framework distinguishes structural corrigibility from ``AI Alignment.'' The AI safety literature has extensively studied alignment (ensuring systems pursue operator-intended goals) \citep{soares2017} and the ``off-switch'' problem \citep{hadfield2017}. These framings center on \textit{operator control}. The safety literature also uses \textit{corrigibility} itself for an operator-facing property: an agent's disposition to accept correction and shutdown from its principal \citep{soares2015}. The two usages are complementary, not competing; this paper's is structural and subject-side. Alignment asks: \textit{Can the operator control the system?} Corrigibility, as used here, asks: \textit{Can the affected subject correct the system?} Structural legitimacy requires the latter.
\end{principle}
...
\begin{enumerate}[noitemsep]
    \item \textbf{Reversibility:} $\forall$ state $s_t \in \mathcal{S}$, $\exists$ transition $s_{t+1}$ (via \textbf{EXIT}) such that $\text{Cost}(s_{t+1}) < \infty$. (No deadlocks).
    \item \textbf{Decidability} (constraint discoverability, not decidability in the computability sense)\textbf{:} $\forall$ input $x$, $\text{Compute}(x) \to \{0,1\}$ is inspectable via \textbf{CODE}. (No hidden constraints).
\end{enumerate}
...
\begin{enumerate}[noitemsep]
    \item \textbf{Subject EXIT.} Affected parties can refuse an AI-mediated determination and obtain a non-AI alternative or a human-authored decision without disproportionate penalty (per the EXIT row of Table~\ref{tab:verification}). This is the canonical EXIT inherited from the deterministic case.
...
\begin{claim}
A learned or agentic deployment failing any of the five EXIT layers cannot satisfy the EXIT test in this framework, irrespective of compliance with CODE, AUDIT, GOVERN, or FORK. The irreducibility argument of Section~\ref{sec:invariance} therefore tightens under agentic conditions: where deterministic infrastructure can fail one EXIT dimension and carry PARTIAL as a diagnostic annotation (the determination itself still resolves PARTIAL to FAIL), agentic infrastructure cannot: failing any layer is total.
\end{claim}
...
\begin{enumerate}[noitemsep]
    \item \textbf{Inference-layer opacity persists.} The boundary does not reach the model's representation. The latent geometry is beyond inspection by design (Section~\ref{subsec:firewall}), and the interpretive categories it realizes are disclosed and governed only by the LWD-R machinery upstream of the boundary (Proposition~\ref{prop:operative-r-test}; R-Layer Change Control). At the boundary you govern what the model \textit{does}; what it \textit{sees with} is governed at the R layer or not at all. Representation Capture Risk (RCR) --- the risk that ontological capture (Section~\ref{sec:code-ai}) becomes irreversible because the categories are neither disclosed nor contestable --- operates upstream of the Action Boundary.
...
When AI decision throughput outruns human legislative correction velocity, the state enters \textbf{Fallout Governance}: policy ceases to guide behavior in advance; it instead reacts post-hoc to scandal, litigation, or failure clusters. Governance becomes damage containment: the Collingridge Dilemma \citep{collingridge1980} at civilizational scale.
...
\paragraph{The Cost of Abandoning Structural Evaluation.} If structural evaluation is set aside on the grounds that ``AI is too stochastic to be evaluated by structural rules,'' governance defers to operators by default: the only remaining standards are those operators choose to publish. Structural evaluation methods are therefore necessary, not optional. The framework does not attempt to inspect the probabilistic ``thoughts'' of AI models. Instead, it demands structural control over the \textit{pipeline that builds them} (data, compute, representational provenance) and the \textit{infrastructure that deploys them} (workflow orchestrations, guardrails, and the operative representation).
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Protocol Documentation}: \texttt{PROTOCOL.md}, \texttt{AGENTS.md}, \texttt{docs/} --- versioning rules, invariants for agent-operated roles, the machine evaluation protocol, and the failure-modes index
...
\begin{itemize}[noitemsep]
    \item \texttt{operative\_r}: The machine form of Proposition~\ref{prop:operative-r-test}. The operator enumerates the deployment variables and publishes the probe set, tolerance, and sample size against which an independent auditor reproduces the deployed classification distribution.
    \item \texttt{executed\_outside\_llm\_context}: The cybernetic anchor. Binds the operator to the claim that governance constraints are hard-coded in the validator's execution environment, not merely injected into the model's context window (which can be bypassed via prompt injection).
    \item \texttt{change\_control}: The velocity constraint of R-Layer Change Control: notice window, suspensive effect, and versioned schema diffs, so drift is itself auditable.
    \item \texttt{scale\_bounds}: Fan-out and action-rate limits as first-class GOVERN instruments (Scale as a Governed Variable).
\end{itemize}
...
The determination structure is deliberate: CODE holds here only because \texttt{operative\_r\_reproduced} is true --- availability of a disclosed schema without behavioral reproduction does not discharge R (Proposition~\ref{prop:operative-r-test}). FORK fails despite a low WCC because training reproduction sits above the community's compute threshold, and \texttt{fork\_viability} records only artifact reproducibility where the \texttt{high\_stakes} tier requires governance reproducibility (Section~\ref{subsec:fork-evidence}).
...
\begin{enumerate}
    \item \textbf{Enforcing GOVERN}: If \texttt{executed\_outside\_llm\_context\_verified} is \texttt{false}, or red-teaming records \texttt{escapes\_found} above zero without documented remediation, the system fails GOVERN. The constraints were probabilistic (advisory to the model) rather than deterministic (binding).

    \item \textbf{Enforcing CODE (LWD-R)}: If \texttt{operative\_r\_reproduced} is \texttt{false}, the system fails the Representation (R) requirement of CODE, triggering Representation Capture Risk (RCR). Publication of a schema without behavioral reproduction is availability, not discharge.

    \item \textbf{Enforcing FORK}: If \texttt{fork\_viability} does not reach the tier's required level, or \texttt{wcc\_score} exceeds the calibrated threshold, the workflow is tightly coupled to a proprietary vendor and the system fails FORK.
\end{enumerate}
...
\texttt{escapes\_found} & GOVERN & $> 0$ unremediated $\Rightarrow$ boundaries are soft \\
\texttt{operative\_r\_reproduced} & CODE & \texttt{false} $\Rightarrow$ R undischarged, RCR triggered \\
\texttt{fork\_viability} & FORK & below tier requirement $\Rightarrow$ vendor lock-in \\
