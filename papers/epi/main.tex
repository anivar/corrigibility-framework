...
\begin{enumerate}[noitemsep]
    ...
    \item \textbf{Bystander EXIT.} Third parties affected by an autonomous agent's actions but not party to its operator-subject relationship (counterparties to an autonomous procurement agent, recipients of an automated communication, persons whose data is incidentally processed, and represented parties acted for under mandate, such as a ward whose guardian operates the wallet that answers for them) require structural recourse independent of the operator-subject loop. The juridical-mechanism families of Section~\ref{subsec:govern-mechanisms}, particularly Injunction Hooks (Figure~\ref{fig:injunction-hook}) and class-action vehicles, are the legitimate sites for bystander EXIT; their absence converts agent autonomy into a unilateral expansion of operator reach.
\end{enumerate}
